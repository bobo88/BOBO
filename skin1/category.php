...
if(!$sort){
	if($keyNum == 1){
		$sql = "SELECT a.id, u.name, a.title, a.summary FROM article AS a,user AS u WHERE a.sort=1 AND a.author=u.id ORDER BY a.id DESC";
	}else if($keyNum == 2){
		$sql = "SELECT a.id, u.name, a.title, a.summary FROM article AS a,user AS u WHERE a.sort=2 AND a.author=u.id ORDER BY a.id DESC";
	}else{
		$sql = "SELECT a.id, u.name, a.title, a.summary FROM article AS a,user AS u WHERE a.sort!=3 AND a.author=u.id ORDER BY a.id DESC";
	}
}else{
	$sql = "SELECT a.id, u.name, a.title, a.summary FROM article AS a,user AS u WHERE a.sort=3 AND a.author=u.id ORDER BY a.id DESC";
}
...
if(!$sort){
	if($keyNum == 1){
		$sql = "SELECT a.id, u.name, a.title, a.time, a.summary FROM article AS a,user AS u WHERE a.sort=1 AND a.author=u.id ORDER BY a.id DESC LIMIT {$pageBegin}, {$pageSize}";
	}else if($keyNum == 2){
		$sql = "SELECT a.id, u.name, a.title, a.time, a.summary FROM article AS a,user AS u WHERE a.sort=2 AND a.author=u.id ORDER BY a.id DESC LIMIT {$pageBegin}, {$pageSize}";
	}else{
		$sql = "SELECT a.id, u.name, a.title, a.time, a.summary FROM article AS a,user AS u WHERE a.sort!=3 AND a.author=u.id ORDER BY a.id DESC LIMIT {$pageBegin}, {$pageSize}";
	}
}else{
	$sql = "SELECT a.id, u.name, a.title, a.time, a.summary FROM article AS a,user AS u WHERE a.sort=3 AND a.author=u.id ORDER BY a.id DESC LIMIT {$pageBegin}, {$pageSize}";
}
